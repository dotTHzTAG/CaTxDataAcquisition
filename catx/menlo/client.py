...
class MenloScanControlClient:
    ...
    async def _reset_averaging(
        ...
        timeout = MenloScanControlClient._averaging_reset_timeout(
        ...
        for attempt in range(attempts):
            ...
            try:
                await MenloScanControlClient._wait_for_averaging_reset(
                    scancontrol, desired_averages, timeout=timeout
                )
                return
            except TimeoutError:
    ...
    @staticmethod
    def _averaging_reset_timeout(scancontrol, desired_averages: int) -> float:
        try:
            rate = float(scancontrol.rate)
        except (TypeError, ValueError):
            rate = 0.0
        if rate > 0:
            return max(3, desired_averages / rate + 2)
        return max(3, desired_averages * 0.02 + 2)

    @staticmethod
    async def _wait_for_averaging_reset(
        scancontrol, desired_averages: int, timeout: float = 3
    ) -> None:
        deadline = asyncio.get_running_loop().time() + timeout
        while asyncio.get_running_loop().time() < deadline:
